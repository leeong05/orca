...
class ComponentsUpdater(UpdaterBase):
    ...
    def pro_update(self):
        ...
        logger.debug('Ensuring index dname_1_date_1 on collection index_components')
    ...
    def update(self, date):
        ...
        if len(df1) == 0:
            logger.warning('No records found for %s on %s', self.db.index_components.name, date)
            return
        ...
        logger.info('UPSERT documents for %d indice into (c: [%s]) of (d: [%s]) on %s', len(grouped), self.db.index_components.name, self.db.name, date)
